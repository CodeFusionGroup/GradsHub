...
public class MyGroupsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        // this code is duplicated here again :( (appears on serverGetUserGroupsResponse() below). It's so that
        // when the user navigates back to My Groups page from the Research Groups page, the recycler view
        // still shows the list of groups. If we remove this code, then the recycler view will be empty when user
        // presses back button from the Research Groups page. (couldn't find easy to understand solution on
        // the internet maybe YOU can :)
        Context context = view.getContext();
        recyclerView = view.findViewById(R.id.list);
        recyclerView.setLayoutManager(new LinearLayoutManager(context));
        adapter = new MyGroupsRecyclerViewAdapter(items, mListener);
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        Button exploreGroupsBtn = view.findViewById(R.id.exploreGroupsBtn);
        exploreGroupsBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                NavController navController = Navigation.findNavController(requireActivity(), R.id.nav_host_fragment);
                navController.navigate(R.id.exploreGroupsFragment);
            }
        });
    }
    ...
    private void serverGetUserGroupsResponse(String output) {
        try {
            ...
            if(jsonObject.has("success")) {
                ...
                String message = joMessage.getString("message");
                Toast.makeText(requireActivity(), message, Toast.LENGTH_SHORT).show();// Toast message: you have not joined any groups.
            }
            ...
            // if the user has joined groups, get the list of research groups this user belongs to.
                ...
                for(int i = 0 ; i < ja.length(); i++) {
                    ...
                    String groupID = jo.getString("RESEARCH_GROUP_ID");
                ...
                Context context = view.getContext(); // context = MainActivity
                recyclerView = view.findViewById(R.id.list);
                recyclerView.setLayoutManager(new LinearLayoutManager(context));
                adapter = new MyGroupsRecyclerViewAdapter(items, mListener);
                recyclerView.setAdapter(adapter);
    ...
    public interface OnListFragmentInteractionListener {
        void onListFragmentInteraction(ResearchGroup item);
    }
